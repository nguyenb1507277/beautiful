...
if(!empty($_POST["login"])) {
	...
	$tendangnhapaddd = trim($_POST['txtTenDangNhap']);
	$matkhau1 =$_POST['txtMatKhau'];
	$matkhau=md5($matkhau1);
	$sql = "Select * from khachhang where kh_tendangnhap = '" .$tendangnhapaddd. "' and kh_matkhau = '" .$matkhau. "' and kh_quantri='1'";
	$result = mysqli_query($conn,$sql);
	$row = mysqli_fetch_array($result);
	if($row) {
			$_SESSION["tendangnhapaddd"] = $row["kh_tendangnhap"];
			...
			if(!empty($_POST["remember"])) {
				...
				$message = "Thất bại! Bạn cần đăng nhập với tài khoản admin";
	}
}
?>	


<?php if(empty($_SESSION["tendangnhapaddd"])) { ?>
...
<?php } 
	...
	if($_SESSION['quantri']==1){
		echo "thanhcong";
		echo '<meta  http-equiv="refresh" content="0;URL=addd.php"/>';
		}
}
